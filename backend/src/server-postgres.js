...
app.post('/api/shifts/:id/attendance', auth, roles('SHIFT_MANAGER', 'SECURITY', 'SYSTEM_ADMIN'), async (req, res, next) => { try { const b = req.body || {}; const row = await one('INSERT INTO attendance(shift_id,department,required_count,present_count,absent_count,late_count,overtime_count,notes) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *', [req.params.id, b.department, b.requiredCount, b.presentCount, b.absentCount, b.lateCount || 0, b.overtimeCount || 0, b.notes || null]); await audit(req.user.sub, 'ATTENDANCE', row.id, 'CREATE', b); const absenceRate = row.required_count ? row.absent_count / row.required_count * 100 : 0; if (row.required_count && absenceRate >= 25) { await pool.query("INSERT INTO notifications(shift_id,severity,title,body) VALUES($1,'CRITICAL',$2,$3)", [req.params.id, 'نسبة غياب مرتفعة جدًا', `${row.department}: نسبة الغياب ${absenceRate.toFixed(0)}% (${row.absent_count} من ${row.required_count})`]); } else if (row.required_count && absenceRate >= 15) { await pool.query("INSERT INTO notifications(shift_id,severity,title,body) VALUES($1,'WARNING',$2,$3)", [req.params.id, 'نسبة غياب مرتفعة', `${row.department}: نسبة الغياب ${absenceRate.toFixed(0)}% (${row.absent_count} من ${row.required_count})`]); } res.status(201).json({ row, attendanceRate: row.required_count ? row.present_count / row.required_count * 100 : 0, absenceRate }); } catch (error) { next(error); } });
...
app.patch('/api/users/:id', auth, roles('SYSTEM_ADMIN'), async (req, res, next) => { try { const b = req.body || {}; if (!b.name || !String(b.name).trim()) return res.status(400).json({ error: 'INVALID_USER_DATA' }); const user = await one('UPDATE users SET name = $1, department = COALESCE($2, department) WHERE id = $3 RETURNING id,name,email,role_code,department,is_active', [String(b.name).trim(), b.department || null, req.params.id]); if (!user) return res.status(404).json({ error: 'USER_NOT_FOUND' }); await audit(req.user.sub, 'USER', user.id, 'NAME_UPDATE', publicUser(user)); res.json({ user: publicUser(user) }); } catch (error) { next(error); } });
